...
class tree(object):
    ...
    """
    def __init__(self, la, X=None, z=None, leafsize=10):
        if not X is None:
            self.tree = cKDTree(X, leafsize=leafsize )
        if not z is None:
            self.z = np.array(z)

    def fit(self, la, X=None, z=None, leafsize=10):
        """
        ...
        """
        return self.__init__(X, z, la, leafsize)

    def __call__(self, X, la, k=6, eps=1e-6, p=2, regularize_by=1e-9):
        """
        Compute the score of query points based on the scores of their k-nearest neighbours,
        weighted by the inverse of their distances.
	...
	#squared distance
        mw = np.sum(weights*np.exp(-la*self.distances), axis=1) / np.sum(np.exp(-la*self.distances), axis=1)
        #mw = np.sum(weights/(self.distances**2), axis=1) / np.sum(1./(self.distances**2), axis=1)
	#exponential
	
        return mw

    def transform(self, X, la, k=6, p=2, eps=1e-6, regularize_by=1e-9):
        """
        Compute the score of query points based on the scores of their k-nearest neighbours,
        weighted by the inverse of their distances.
        ...
        """
        return self.__call__(X, k, eps, p, regularize_by,la)
